 ...
 */
export class CustomSnakeCaseNamingStrategy implements NamingStrategyContract {
  /**
  ...
  public relationForeignKey(
    ...
    if (relation === 'belongsTo') {
      return `${relatedModel.name}_${relatedModel.primaryKey}`.toLowerCase()
    }

    return `${model.name}_${model.primaryKey}`.toLowerCase()
  }
